...
int main(int argc, char *argv[]){
    ...
    if (argc < 3) {
        printf("Wrong argment! [ori_file_loc] [length bitwidth=15] [out file dir]\n");
        return -1;
    }
    filename = argv[1];
    ...
    unsigned short f_maxlength=0;
    unsigned short O_tile=0;
    unsigned short Maxlength=0;
    double maxlength_double = (double)maxlength_i;
    Maxlength=((unsigned short)pow(2.0,maxlength_double)-1);
    ...
    for(i=1;i<size;i++){
        ...
        if(lsb[i]==prevalue){
            ...
            O_tile = (prevalue<<15)|length;
            fwrite(&O_tile,sizeof(unsigned short),1,fp_o);
            length=1;
            if(prevalue==0){
                prevalue=1;
            }
