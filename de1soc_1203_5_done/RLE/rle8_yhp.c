...
int main(int argc, char *argv[]){
    ...
    //char filename_o_temp[99]="./compressed/compressed_";
    char *str_compressed;
    // for iteration
    int i = 1;
    ...
    int val=0;
    if (argc < 1) {
        printf("Wrong argment! [ori_file_loc] [length bitwidth] [out file dir_name(it will make dir)]\n");
        return -1;
    }
    filename = argv[1];
    ...
    unsigned char f_maxlength=0;
    unsigned char O_tile=0;
    unsigned char Maxlength=0;
    double maxlength_double = (double)maxlength_i;
    Maxlength=((unsigned char)pow(2.0,maxlength_double)-1);
    ...
    for(i=1;i<size;i++){
        ...
        if(lsb[i]==prevalue){
            ...
            O_tile = (prevalue<<7)|length;
            fwrite(&O_tile,sizeof(unsigned char),1,fp_o);
            length=1;
            if(prevalue==0){
                prevalue=1;
            }
